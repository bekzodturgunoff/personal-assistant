...
export function createGeminiBrainProvider(): BrainProvider {
  return {
    async analyze(
      ...
      console.log(`[Brain/Gemini] Analyzing conversation (${history.length} entries)...`);
      ...
      try {
        ...
        console.error("[Brain/Gemini] Analysis failed:", err);
